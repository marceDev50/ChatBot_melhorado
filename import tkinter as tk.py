...
class SimpleChatbot:
    ...
    def get_response(self, user_input):
        ...
        if "calcule" in user_input or "calcula" in user_input:
    ...
    def calculate(self, user_input):
        try:
            # Remove "calcule" ou "calcula" do input do usuário e qualquer espaço extra
            expression = user_input.replace("calcule", "").replace("calcula", "").strip()
            # Avalia a expressão matemática
            result = eval(expression)
            return f"O resultado é: {result}"
        except Exception as e:
            return "Desculpe, não consegui calcular isso. Verifique a expressão e tente novamente."
